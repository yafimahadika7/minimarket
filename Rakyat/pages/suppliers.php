...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && $id_from_url && $is_connected) {
    $redirect_to_self = "Location: suppliers.php";

    $stmt = $koneksi->prepare("DELETE FROM $DB_TABLE WHERE $ID_COLUMN = ?");
    if ($stmt) {
        $stmt->bind_param("i", $id_from_url);
        if ($stmt->execute()) {
            $_SESSION['crud_message'] = '<div class="alert alert-success">Supplier berhasil dihapus.</div>';
        } else {
...
if ($is_connected && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $post_id = $_POST['supplier_id'] ?? null;
    $nama_s = trim($_POST['nama_supplier']);
    $alamat_s = trim($_POST['alamat']);
    $telp_s = trim($_POST['no_telepon']);
    $kontak_s = trim($_POST['nama_kontak']);
    $submit_type = $_POST['submit_type'] ?? 'save'; 
    
    if (empty($nama_s)) {
        $_SESSION['crud_message'] = '<div class="alert alert-danger">Nama Supplier tidak boleh kosong.</div>';
        header($redirect_to_self);
        exit;
    }
    ...
    if ($submit_type == 'save' && empty($post_id)) {
        $sql_insert = "INSERT INTO $DB_TABLE (nama_supplier, alamat, no_telepon, nama_kontak) VALUES (?, ?, ?, ?)";
        $stmt = $koneksi->prepare($sql_insert);

        if ($stmt) {
            $stmt->bind_param("ssss", $nama_s, $alamat_s, $telp_s, $kontak_s);
            if ($stmt->execute()) {
                $_SESSION['crud_message'] = '<div class="alert alert-success">Supplier berhasil ditambahkan.</div>';
            } else {
                $error_msg = $stmt->errno == 1062 ? "Nama Supplier sudah ada." : $stmt->error;
        ...
        $sql_update = "UPDATE $DB_TABLE SET nama_supplier=?, alamat=?, no_telepon=?, nama_kontak=? WHERE $ID_COLUMN=?";
        $stmt = $koneksi->prepare($sql_update);
        
        if ($stmt) {
            $stmt->bind_param("ssssi", $nama_s, $alamat_s, $telp_s, $kontak_s, $post_id);
            
            if ($stmt->execute()) {
                $_SESSION['crud_message'] = '<div class="alert alert-success">Data supplier berhasil diubah.</div>';
            } else {
                $error_msg = $stmt->errno == 1062 ? "Nama Supplier sudah ada." : $stmt->error;
                $_SESSION['crud_message'] = '<div class="alert alert-danger">Gagal mengubah data: '. htmlspecialchars($error_msg) .'</div>';
            }
            $stmt->close();
        } else {
            $_SESSION['crud_message'] = '<div class="alert alert-danger">Prepared statement error: '. htmlspecialchars($koneksi->error) .'</div>';
...
if ($id_from_url && $is_connected) {
    $sql_get = "SELECT $ID_COLUMN, nama_supplier, alamat, no_telepon, nama_kontak FROM $DB_TABLE WHERE $ID_COLUMN=?";
    ...
    if ($stmt) {
        ...
        if ($res && $res->num_rows == 1) {
            ...
            $message .= '<div class="alert alert-danger">Data supplier tidak ditemukan.</div>';
            $id_from_url = null;
        }
        $stmt->close();
    } else {
...
$supplier_list = [];
if ($is_connected) {
    $sql = "SELECT $ID_COLUMN, nama_supplier, alamat, no_telepon, nama_kontak FROM $DB_TABLE ORDER BY $ID_COLUMN DESC";
    ...
    if ($res) {
        ...
        $message .= '<div class="alert alert-danger">Gagal memuat data supplier: '. htmlspecialchars($koneksi->error) .'</div>';
    }
}
?>
<!DOCTYPE html>
...
<title>Data Supplier | Minimarket App</title>
...
<nav id="sidebar">
    ...
    <ul class="nav flex-column p-2">
        ...
        <li class="nav-item">
            <a class="nav-link active-page" href="suppliers.php"><i class="fas fa-truck"></i> Data Supplier</a>
        </li>
        <li class="nav-item">
...
<h2 class="fw-bold text-dark"><i class="fas fa-truck me-2"></i> Data Supplier</h2>
<span class="text-muted">Manajemen Data Pemasok (Role: <b><?= strtoupper($user_role); ?></b>)</span>
</div>
<?= $message ?>
<div class="row">
    <div class="col-md-4">
        <div class="card mb-4">
            <div class="card-header bg-warning text-dark">
                <h5 class="mb-0"><i class="fas <?= $form_mode=='edit'?'fa-edit':'fa-plus'; ?>"></i> <?= $form_mode=='edit'?'Ubah':'Input'; ?> Data Supplier</h5>
            </div>
            <div class="card-body">
                <form id="supplierForm" method="POST" action="suppliers.php">
                    <input type="hidden" name="supplier_id" value="<?= htmlspecialchars($data['supplier_id']) ?>">
                    <input type="hidden" name="submit_type" value="<?= $form_mode=='edit'?'update':'save'; ?>">
                    
                    <div class="mb-2">
                        <label class="form-label">ID Supplier</label>
                        <input type="text" class="form-control" value="<?= $data['supplier_id'] ? $ID_PREFIX.str_pad($data['supplier_id'],4,'0',STR_PAD_LEFT) : '(Auto)'; ?>" readonly>
                    </div>
                    <div class="mb-2">
                        <label class="form-label">Nama Perusahaan Supplier</label>
                        <input type="text" class="form-control" name="nama_supplier" required value="<?= htmlspecialchars($data['nama_supplier']) ?>">
                    </div>
                    <div class="mb-2">
                        <label class="form-label">Nama Kontak Person</label>
                        <input type="text" class="form-control" name="nama_kontak" value="<?= htmlspecialchars($data['nama_kontak']) ?>">
                    </div>
                    <div class="mb-2">
                        <label class="form-label">No. Telepon</label>
                        <input type="text" class="form-control" name="no_telepon" value="<?= htmlspecialchars($data['no_telepon']) ?>">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="d-flex justify-content-between gap-2">
                        ...
                        <?php if($form_mode=='edit'): ?>
                            <a href="suppliers.php?action=delete&id=<?= $data['supplier_id'] ?>" 
                               class="btn btn-danger flex-fill" 
                               onclick="return confirm('Yakin hapus supplier <?= htmlspecialchars($data['nama_supplier']) ?>? Tindakan ini tidak dapat dibatalkan.')">
                                <i class="fas fa-trash"></i> Hapus
                            </a>
                        <?php endif; ?>
                    </div>
                    <?php if($form_mode=='edit'): ?><a href="suppliers.php" class="btn btn-secondary w-100 mt-2"><i class="fas fa-plus"></i> Form Baru</a><?php endif; ?>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between bg-dark text-white">
                ...
                <div class="d-flex">
                    <input type="text" id="searchInput" class="form-control form-control-sm me-2" placeholder="Cari Supplier...">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover table-striped mb-0 align-middle" id="supplierTable">
                        <thead class="table-light">
                        <tr>
                            <th onclick="sortTable(0)" style="cursor:pointer;">ID <i class="fas fa-sort"></i></th>
                            <th onclick="sortTable(1)" style="cursor:pointer;">Nama Supplier <i class="fas fa-sort"></i></th>
                            <th onclick="sortTable(2)" style="cursor:pointer;">Kontak Person <i class="fas fa-sort"></i></th>
                            <th onclick="sortTable(3)" style="cursor:pointer;">No. Telepon <i class="fas fa-sort"></i></th>
                            <th>Alamat</th>
                        </tr>
                        </thead>
                        <tbody id="supplierBody">
                        <?php if($is_connected && !empty($supplier_list)):
                        foreach($supplier_list as $s):
                        ?>
                        <tr onclick="window.location.href='suppliers.php?id=<?= $s['supplier_id'] ?>'" 
                            style="cursor:pointer; <?= ($id_from_url==$s['supplier_id'])?'background-color:#fff3e6;font-weight:bold;':''; ?>">
                            
                            <td><?= $ID_PREFIX.str_pad($s['supplier_id'],4,'0',STR_PAD_LEFT) ?></td>
                            <td><?= htmlspecialchars($s['nama_supplier']) ?></td>
                            <td><?= htmlspecialchars($s['nama_kontak']) ?></td>
                            <td><?= htmlspecialchars($s['no_telepon']) ?></td>
                            <td><?= htmlspecialchars($s['alamat']) ?></td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr><td colspan="5" class="text-center"><?= $is_connected?'Tidak ada data supplier.':'Gagal menampilkan data karena koneksi database bermasalah.' ?></td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
...
const tableBody=document.getElementById("supplierBody"),searchInput=document.getElementById("searchInput");
...
function sortTable(colIndex){
    ...
    visibleRows.sort((a,b)=>{
        ...
        if(colIndex===0){
            ...
            const id_a=parseInt(x.replace('s','')),
                  id_b=parseInt(y.replace('s',''));
